...
%
% REQUIERED FILES: scratchDetection.m, brushDetection.m, 1785125303951_trainedInspectAI.mat
%
% TO RUN THIS CODE
...
%   .finalLabel            - 'PASS' or 'FAIL' (from AI classifier)
...
%
    ...
    [scratchMask, scratchProps] = scratchDetection(image, showDebug);
    [brushMask,   brushProps]   = brushDetection(image, showDebug);
    ...
    finalLabel       = mapToPassFail(aiLabelStr, classNames);
    ...
    disagreementFlag       = ~strcmp(finalLabel, baselineDecision);
    ...
    fprintf('AI Decision:       %s (confidence %.2f)\n', finalLabel, confidenceScore);
    fprintf('Rule-Based Backup: %s (scratchFail=%d, brushFail=%d)\n', ...
        baselineDecision, scratchFail, brushFail);

    if disagreementFlag
        fprintf('*** DISAGREEMENT: AI and rule-based baseline do not match ***\n');
    end
